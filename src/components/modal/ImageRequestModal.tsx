...
async function loadResults(cesium: any, outputs: any[], jobName: string, jobId: string, setShowCredsExpiredAlert: any) {
    for (const output of outputs) {
        if (output.type === "S3") {
            console.log("Loading results from S3");
            await loadS3GeoJson(cesium, output.bucket, `${jobName}/${jobId}.geojson`, setShowCredsExpiredAlert)
        }
    }
}
...
const NewRequestModal = ({
    ...
    const [bucketStatus, setBucketStatus] =useState<DropdownStatusProps.StatusType>("pending");
    const [imageValue, setImageValue] =useState("");
    const [imageStatus, setImageStatus] =useState<DropdownStatusProps.StatusType>("pending");
    const [modelValue, setModelValue] = useState("");
    const [modelStatus, setModelStatus] =useState<DropdownStatusProps.StatusType>("pending");
    ...
    useEffect(() => {
        const getData = async (cesium: any, outputs: any, jobName: string, jobId: string) => {
            await loadResults(cesium, outputs, jobName, jobId, setShowCredsExpiredAlert);
        }
        ...
        <div>
            <div>
                <Modal
                    ...
                    header={
                        <Fragment>Create Image Request</Fragment>
                    }
                >
                    {showCredsExpiredAlert &&
                        ...
                        <Form
                            actions={
                                <SpaceBetween direction="horizontal" size="xs">
                                    <Button onClick={() => setShowImageRequestModal(false)} formAction="none"
                                            variant="link">
                                        Cancel
                                    </Button>
                                    <Button onClick={async () => {
                                        ...
                                        await loadImageInCesium(cesium, bucketValue, imageValue, imageId, setShowCredsExpiredAlert);
                                    }
                                    }
                                            variant="primary">
                                        Submit
                            ...
                            <Container>
                                <SpaceBetween direction="vertical" size="l">
                                    ...
                                    <FormField label="Tile Compression">
                                        <Autosuggest
                                            onChange={({detail}) => setCompressionValue(detail.value)}
                                    ...
                                    <FormField label="Outputs">
                                        <Multiselect
                                            ...
                                            onChange={({detail}) => {
                                                if (detail.selectedOptions && Array.isArray(detail.selectedOptions)) {
