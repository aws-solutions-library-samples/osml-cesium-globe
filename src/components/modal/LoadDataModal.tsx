...
const LoadDataModal = ({
    ...
    showLoadDataModal: any, setShowLoadDataModal: any,
    ...
    const displayData = () => {
        if (activeTabId == "S3") {
            if (s3Bucket && s3Object && cesium.viewer) {
                loadS3GeoJson(cesium, s3Bucket, s3Object, setShowCredsExpiredAlert)
            ...
            if (localFile && cesium.viewer) {
                ...
                loadGeoJson(cesium.viewer, features)
                ...
                tabs={[
                    {
                        label: "From S3",
                        id: "S3",
                        content: (<S3DataSelector s3Object={s3Object}
                    ...
                    },
                    {
                        label: "From Local",
                        id: "local",
                        content: (<LocalDataSelector localFile={localFile} setLocalFile={setLocalFile}/>)
                    }
                ]}
            />
        </Modal>
