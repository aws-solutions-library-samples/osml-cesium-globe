...
# Constants
DOCKER_IMAGE = "tumgis/ctb-quantized-mesh:alpine"
...
def main(directory, image, start_zoom, end_zoom):
    # Validate inputs
    ...
    terrain_output_folder = f"terrain_{image_name}_{timestamp}"
    ...
    try:
        ...
        container = client.containers.run(DOCKER_IMAGE, "tail -f /dev/null", name=JOB_NAME, detach=True, volumes={directory: {'bind': '/data', 'mode': 'rw'}})

        # Process image
        exec_command = lambda cmd: container.exec_run(cmd, tty=True)
        
        exec_command(f"mkdir -p {terrain_output_folder}")
        exec_command(f"ctb-tile -f Mesh -C -N -s {start_zoom} -e {end_zoom} -o {terrain_output_folder} {image}")
        exec_command(f"ctb-tile -f Mesh -l -C -N -o {terrain_output_folder} {image}")
    ...
    unzip_cmd = f"find {os.path.join(directory, terrain_output_folder)} -type f -name '*.terrain' -exec bash -c 'mv \"$0\" \"$0.gz\" && gunzip \"$0.gz\"' {{}} \\;"
